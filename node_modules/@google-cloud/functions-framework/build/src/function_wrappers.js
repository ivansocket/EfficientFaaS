...
const cache_proxy = require("../src/cache_proxy")
/**
    ...
    return (req, res) => {
        ...
        d.run(async () => {
            process.nextTick(async () => {
                ...
                req.body[propertyAccessTest.property] = req.body[propertyAccessTest.property]
                .toString().replace("data:image/jpeg;base64,", "")

                req.hash_key = crypto.createHash('sha256')
                .update(req.body[propertyAccessTest.property].toString())
                .digest('hex');

                req.cachingClient = await cache_proxy.ConnectCache(req, propertyAccessTest.cachingService)

                functionResult = await functions_manager.FunctionExecutionDecider(execute, req, res, cache_proxy)
            });
        });
        res.status(200).send(functionResult)
    };
};
/**
